defmodule Coherence.CredentialStore.Server.Test do
  ...
  defp delete_credentials(state, credentials) do
    {:noreply, state1} = Server.handle_cast({:delete_credentials, credentials}, state)
    state1
  end

  defp get_user_data(state, credentials) do
    {:reply, data, state1} = Server.handle_call({:get_user_data, credentials}, nil, state)
    {state1, data}
  end

  defp update_user_logins(state, user_data) do
    {:noreply, state1} = Server.handle_cast({:update_user_logins, user_data}, state)
    state1
  end

  defp delete_user_logins(state, user_data) do
    {:noreply, state1} = Server.handle_cast({:delete_user_logins, user_data}, state)
    state1
  end
